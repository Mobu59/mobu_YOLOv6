    ...
    def forward(self, x):
        ...
        #        axis=-1)
            ...
            for i in range(self.nl):
                ...
                cls_output_af = torch.sigmoid(cls_output_af)
                if deploy:
                    cls_score_list_af.append(cls_output_af.reshape([b, 1, self.nc, l]))
                    reg_dist_list_af.append(reg_output_af.reshape([b, 1, 4, l]))
                else:
                    cls_score_list_af.append(cls_output_af.reshape([b, self.nc, l]))
                    reg_dist_list_af.append(reg_output_af.reshape([b, 4, l]))
                
            if deploy: 
                cls_score_list_af = torch.cat(cls_score_list_af, axis=3).permute(0, 1, 3, 2)
                reg_dist_list_af = torch.cat(reg_dist_list_af, axis=3).permute(0, 1, 3, 2)
                mobile = True
                if mobile:
                    lt = reg_dist_list_af[..., :2] - 0.5
                    rb = reg_dist_list_af[..., 2:] + 0.5
                    c_xy = (rb - lt) / 2
                    wh = torch.log((rb + lt))
                    pred_bboxes = torch.cat([c_xy, wh], 3)
                    #obj_score_list, class_pred = torch.max(cls_score_list_af, 3, keepdim=True)
                    cls_score_list, class_pred = torch.max(cls_score_list_af, 3, keepdim=True)
                    #cls_score_list = cls_score_list_af
                    #debug
                    #anchor_points_af, stride_tensor_af = generate_anchors(
                    #    x, self.stride, self.grid_cell_size, 0, device=x[0].device, is_eval=True, mode='af')
                    #print(pred_bboxes[..., :2] + anchor_points_af) 
                    #print(torch.exp(pred_bboxes[..., 2:4]))
                    #exit()
                else:
                    anchor_points_af, stride_tensor_af = generate_anchors(
                        x, self.stride, self.grid_cell_size, 0, device=x[0].device, is_eval=True, mode='af')
                    lt, rb = torch.split(reg_dist_list_af, 2, -1) 
                    lt -= 0.5
                    rb += 0.5
                    x1y1 = anchor_points_af - lt 
                    x2y2 = anchor_points_af + rb 
                    c_xy = (x1y1 + x2y2) / 2
                    wh = x2y2 - x1y1
                    bbox = torch.cat([c_xy, wh], -1)
                    bbox *= stride_tensor_af
                    pred_bboxes = bbox
                    cls_score_list = cls_score_list_af
                    obj_score_list = torch.ones((b, pred_bboxes.shape[1], 1), device=pred_bboxes.device, dtype=pred_bboxes.dtype),
